...
public class Ants {
    ...
    public Ilk getIlk(Tile tile) {
        return map[tile.getRow()][tile.getColumn()];
    }
    ...
    public void setIlk(Tile tile, Ilk ilk) {
        map[tile.getRow()][tile.getColumn()] = ilk;
    }
    ...
    public Ilk getIlk(Tile tile, Aim direction) {
        ...
        return map[newTile.getRow()][newTile.getColumn()];
    }
    ...
    public Tile getTile(Tile tile, Aim direction) {
		int row = getRow(tile, direction.getRowDelta());
		int col = getColumn(tile, direction.getColDelta());
		return new Tile(row, col);
	}

	public int getColumn(Tile tile, int columnDelta) {
		int col = (tile.getColumn() + columnDelta) % cols;
        if (col < 0) {
            col += cols;
        }
		return col;
	...
	public int getRow(Tile tile, int rowDelta) {
    ...
    public Tile getTile(Tile tile, Tile offset) {
        ...
        int col = (tile.getColumn() + offset.getColumn()) % cols;
        if (col < 0) {
            col += cols;
        }
        return new Tile(row, col);
    ...
    public List<Aim> getDirections(Tile t1, Tile t2) {
        ...
        if (t1.getColumn() < t2.getColumn()) {
            if (t2.getColumn() - t1.getColumn() >= cols / 2) {
                directions.add(Aim.WEST);
            } else {
                directions.add(Aim.EAST);
            }
        } else if (t1.getColumn() > t2.getColumn()) {
            if (t1.getColumn() - t2.getColumn() >= cols / 2) {
                directions.add(Aim.EAST);
            } else {
                directions.add(Aim.WEST);
            }
    ...
    public void clearMyAnts() {
        for (Tile myAnt : myAnts) {
            map[myAnt.getRow()][myAnt.getColumn()] = Ilk.LAND;
        }
        myAnts.clear();
    }
    ...
    public void clearEnemyAnts() {
        for (Tile enemyAnt : enemyAnts) {
            map[enemyAnt.getRow()][enemyAnt.getColumn()] = Ilk.LAND;
        }
        enemyAnts.clear();
    }
    ...
    public void clearFood() {
        for (Tile food : foodTiles) {
            map[food.getRow()][food.getColumn()] = Ilk.LAND;
        }
        foodTiles.clear();
    }
    ...
    public void setVision() {
        for (Tile antLoc : myAnts) {
            for (Tile locOffset : visionOffsets) {
                ...
                visible[newLoc.getRow()][newLoc.getColumn()] = true;
            }
        }
    }
